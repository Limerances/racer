...
MASTER_PORT_BASE="${MASTER_PORT_BASE:-${MASTER_PORT:-29500}}"
...
RACER_PAYLOAD_POOL_PREWARM_CHUNKS="${RACER_PAYLOAD_POOL_PREWARM_CHUNKS:-0}"
...
wait_for_save_then_kill() {
  ...
  start="$(date +%s)"
  while true; do
    if grep -q "RACER save blocking time: iteration=${target_iter}" "${log_path}" 2>/dev/null; then
