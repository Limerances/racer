...
MASTER_PORT="${MEGATRON_MASTER_PORT:-${MASTER_PORT:-29500}}"
# PAI exports MASTER_PORT/PET_MASTER_PORT for the platform launcher. If that value
# leaked into this script, use a Megatron-owned default unless explicitly overridden.
if [[ -n "${PET_MASTER_PORT:-}" && "${MASTER_PORT}" == "${PET_MASTER_PORT}" && -z "${MEGATRON_MASTER_PORT:-}" ]]; then
  MASTER_PORT=29500
fi
...
if [[ -z "${MEGATRON_DATALOADER_PIN_MEMORY}" ]]; then
  case "${MODE}" in
    racer_egm|racer_egm_remote_spare)
      MEGATRON_DATALOADER_PIN_MEMORY=0
      ;;
    *)
      MEGATRON_DATALOADER_PIN_MEMORY=1
      ;;
  esac
fi
export MEGATRON_DATALOADER_PIN_MEMORY
...
export CSD_EGM_SEGMENT_BYTES
export CSD_EGM_MAX_POOL_BYTES
export RACER_DEBUG_STORAGE_READ_CHECKSUM
...
if [[ "${MODE}" != "baseline" ]]; then
  ...
  RACER_EXTENSION_BUILD_DIR="${RACER_EXTENSION_BUILD_DIR:-${RACER_ROOT}/.cache/torch_extensions/racer_cuda_ext/${RACER_EXTENSION_CACHE_KEY}}"
...
prepare_racer_cuda_extension() {
  ...
  if [[ "${NODE_RANK}" == "${RACER_EXTENSION_COORDINATOR_NODE_RANK}" ]]; then
    if [[ "${RACER_EXTENSION_TRUST_READY}" == "1" && -f "${RACER_EXTENSION_READY_FILE}" ]]; then
      echo "RACER CUDA extension ready marker exists: ${RACER_EXTENSION_READY_FILE}"
      return
    fi
...
PY
  ...
  if [[ "${RACER_EXTENSION_TRUST_READY}" == "1" ]]; then
    echo "RACER CUDA extension ready marker accepted: ${RACER_EXTENSION_READY_FILE}"
    return
  fi
...
MSG
  ...
  if [[ "${RACER_DISTRIBUTED_DIRECT_STORAGE_LOAD}" == "auto" ]]; then
    if [[ "${RACER_CSD_PER_NODE}" == "1" && "${RACER_CSD_LOCAL_RANKS_EXPANDED}" != "${RACER_TRAIN_RANKS_EXPANDED}" ]]; then
      RACER_DISTRIBUTED_DIRECT_STORAGE_LOAD=0
    else
      RACER_DISTRIBUTED_DIRECT_STORAGE_LOAD=1
    fi
  fi
  export RACER_CSD_PER_NODE
  export RACER_CSD_LOCAL_RANKS
  export RACER_CSD_LOCAL_COORDINATOR_RANK
  export RACER_DISTRIBUTED_DIRECT_STORAGE_LOAD
  export RACER_DEBUG_PAYLOAD_CHECKSUM
  export RACER_DEBUG_STORAGE_READ_CHECKSUM
...
run_remote_spare_worker() {
  ...
  echo "RACER_DEBUG_PAYLOAD_CHECKSUM=${RACER_DEBUG_PAYLOAD_CHECKSUM:-}"
  echo "RACER_DEBUG_STORAGE_READ_CHECKSUM=${RACER_DEBUG_STORAGE_READ_CHECKSUM:-}"
  ...
  echo "RACER_EXTENSION_TRUST_READY=${RACER_EXTENSION_TRUST_READY}"
  ...
  echo "CSD_EGM_TOTAL_BYTES=${CSD_EGM_TOTAL_BYTES}"
  echo "CSD_EGM_SEGMENT_BYTES=${CSD_EGM_SEGMENT_BYTES}"
  echo "CSD_EGM_MAX_POOL_BYTES=${CSD_EGM_MAX_POOL_BYTES}"
