...
def _payload_store_slot(payload: torch.Tensor, nbytes: int, *, device: torch.device) -> torch.Tensor:
...
def _store_data_rows_for_group(
    *,
    ...
    for slot in group:
        ...
        if rank == src_rank:
            payload = _payload_store_slot(local_slot_payload[src_rank], nbytes, device=device)
            if owner == rank:
...
def distributed_store(
    *,
    ...
    def slot3_view(nbytes: int) -> torch.Tensor:
        ...
        return _payload_store_slot(local_payload, int(nbytes), device=_current_cuda_device())
    ...
    def receive_slot_for(group: Sequence[Any], *, after_source_send: bool) -> torch.Tensor | None:
        ...
        if local_payload is not None and (source_group_completed or after_source_send):
