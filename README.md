...
- Preserves Jerasure-compatible GF(2^8) Cauchy Reed-Solomon semantics: `k + m == len(train_ranks)`. Spare ranks are compute/storage accelerators and are not rows in the E matrix.
...
For a quick CPU smoke test without materializing the full GPT2 checkpoint:

```bash
cd /workspace/racer_o2
python examples/bench_gpt2_checkpoint.py \
  --backend cpu \
  --storage-backend in_process_cpu \
